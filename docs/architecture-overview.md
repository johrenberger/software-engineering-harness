...
| Real MiniMax adapter | **DONE** (clusters M3-1..M3-4) — HTTP transport, default OpenAI-compatible endpoint `https://api.minimax.io/v1/chat/completions`; **production model is `MiniMax-M3`** with no silent fallback; capability-based readiness; live-smoke gated on `RUN_MINIMAX_LIVE_TEST`; native endpoint also supported via `MiniMaxAdapter.native_chatcompletion_v2`; offline vertical-acceptance test (`tests/e2e/test_m3_offline_vertical.py`) exercises the full spec→plan→impl→review pipeline against a fixture repository | `src/seharness/models/minimax.py`, `src/seharness/models/minimax_transport.py`, `src/seharness/models/minimax_m3_composition.py`, `src/seharness/models/readiness_validation.py` |
...
| Vertical acceptance evidence | **DONE** (cluster M3) — `docs/vertical-acceptance.md` is the current M3 vertical-acceptance index; `docs/m3-capability-matrix.md` distinguishes component-vs-integrated capability per phase; `docs/vertical-acceptance-cluster-n.md` is preserved as historical M2.7 transport evidence (do not present as current-model acceptance); live-deployed vertical remains a recorded gap closed via M3-5 Option (a) | `docs/vertical-acceptance.md`, `docs/m3-capability-matrix.md`, `docs/vertical-acceptance-cluster-n.md` |
